...
def check_vpn():
    p = subprocess.Popen('cmd.exe /c' + 'F:\\uploadPinterestPic\\boot\\checkvpn.bat abc',
...
def rasphone_vpn():
    p = subprocess.Popen("cmd.exe /c" + 'F:\\uploadPinterestPic\\boot\\rasphonevpn.bat abc',
                         stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    curline = p.stdout.readline()
    while curline != b'':
        # print(curline)
...
def write_txt_time():
    ...
    with open('F:\\uploadPinterestPic\\boot\\config_time.txt', 'w', encoding='utf-8') as fp:
        fp.write(time_str)
...
def connect_vpn(conn, vpn):
    ...
    if result:
        ...
        with open("F:\\uploadPinterestPic\\boot\\vpn.txt", "w", encoding='utf-8') as fp:
